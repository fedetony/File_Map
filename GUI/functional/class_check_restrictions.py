...
cres_fun_name = "Check Restrictions"
# Add logger
try:
    from class_LogHandler import get_appPath, LM
    log = LM.get_logger_with_handler(cres_fun_name,
                                     "debug",
                                     False,
                                     "%(asctime)s [%(levelname)s] (%(name)s) %(message)s")
    log.info(f"{cres_fun_name} Logger started")
except (AttributeError, ImportError):
    # set up logging to file - see previous section for more details
    log = logging.getLogger(cres_fun_name) #'' for root logger
    ...
    log.info(f"{cres_fun_name} Local Logger started")
