...
try:
    ...
    log = logging.getLogger(tw_fun_name) #'' for root logger
    ...
    log.info(f"{tw_fun_name} Local Logger started")
...
class TableWidgetFunctions(QtWidgets.QWidget):
        ...
        if(event.type() == QtCore.QEvent.Type.MouseButtonPress and
           ...
           source is self.tablewidgetobj.viewport()):
            ...
            if item is not None:
                ...
                self.signal_item_button_right_clicked.emit(track,event.globalPosition().toPoint())
    ...
    def _item_combobox_indexchanged(self, cbw: QtWidgets.QComboBox, track: list = []):
        ...
        index = cbw.findText(currenttxt, QtCore.Qt.MatchFlag.MatchFixedString)
    ...
    def _set_checkbox_value_to_item(self, valueitem: QtWidgets.QTableWidgetItem):
        ...
        if self.check_restrictions.str_to_bool(valueitem.text()):
            self._set_checkbox_value(valueitem,True)
        else:
            self._set_checkbox_value(valueitem,False)
    ...
    def create_data_model_tablewidget(
        ...
        for iii, t_item in enumerate(self.table_items):
            model.setHeaderData(iii, QtCore.Qt.Orientation.Horizontal, t_item)
...
class Delegate(QtWidgets.QStyledItemDelegate):
    ...
    def setModelData(self, editor, model, index):
        if index.column() == self.itm_col and index.row() == self.itm_row:
            ...
            model.setData(index, option, QtCore.Qt.ItemDataRole.DisplayRole)
