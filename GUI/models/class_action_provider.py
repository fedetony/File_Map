...
class BrowseFileActionProvider(TreeActionProvider):
    icons=Icons()
    max_depth_count = 100
    mode="files"
    hide_files=False
    ...
    def set_hide_files(self,hide_files):
    ...
    def _expand_tree(self,node: TreeNode, count = 0 ):
        ...
        if count > self.max_depth_count:
            return count
        if self.parent_widget:
            # if hasattr(self.parent_widget,"load_node"):
            #     if node.i_am == "dir":
            #         # load node and its children
            #         self.parent_widget.load_node(node, max_level=node.level+1)
            if hasattr(self.parent_widget,"expand_node"):
                if node.i_am == "dir":
                    if not node.loaded:
                        count += 1
                    ...
                    for child in node.children:
                        count = self._expand_tree(child,count)
                elif node.i_am == "file" and self.mode =="dirs":
                    node.hidden = self.hide_files
            
            
        return count
    
    def hide_all_files(self, tree_manager: TreeManager,hide_files:bool):
        """hides/shows all files loaded"""
        all_file_nodes=tree_manager.get_nodes_by_attribute("i_am","file")
        self.set_hide_files(hide_files)
                ...
                if node.i_am == "dir":
                    ...
                    for child in node.children:
                        count = self._select_tree(child,
                                                  ...
                                                  selected=selected,
                                                  count=count)                    
                elif node.i_am == "file" and self.mode == "files":
                    tree_manager.set_selected(node,selected)   
                elif node.i_am == "file" and self.mode == "dirs":
                    node.hidden = self.hide_files
        return count       
